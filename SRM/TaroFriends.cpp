...
int getNumber(vector <int> c, int X)
{
    ...
    rep(i,0,l)
    {
        ...
        {
            //i+,j+
            int x1=c[i]+X,x2=c[j]+X;
            int mn=min(x1,x2);
            int mx=max(x1,x2);
            cnt=0;
            rep(k,0,l)
            {
                if((c[k]+X>=mn&&c[k]+X<=mx)||(c[k]-X>=mn&&c[k]-X<=mx))
                cnt++;
            }
            if(cnt==l)
            ret=min(ret,mx-mn);
...
//	template <typename T> string print_array(const vector<T> &V) { ostringstream os; os << "{ "; for (typename vector<T>::const_iterator iter = V.begin(); iter != V.end(); ++iter) os << '\"' << *iter << "\","; os << " }"; return os.str(); }
	...
	void test_case_0() { int Arr0[] = {1, 2, 5}; vector <int> Arg0(Arr0, Arr0 + (sizeof(Arr0) / sizeof(Arr0[0]))); int Arg1 = 10; int Arg2 = 4; verify_case(0, Arg2, getNumber(Arg0, Arg1)); }
